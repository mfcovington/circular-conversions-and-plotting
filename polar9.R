...
polar.plot <- function(r, theta, grp = NULL, pch = NULL, col = NULL,hours = TRUE,avg=TRUE, angle.axis = -90,reverse=TRUE) {
  ...
  pplot <- function(r,theta,pch=NULL,col=NULL){
    ...
    points(r * cos(theta), r * sin(theta), col = col, pch = pch,cex=2) #put the points on the graph
  }
  ...
  arrows(0,0,mean(r)*cos(mean(theta)),mean(r)*sin(mean(theta)),lwd=4, col=col)
  }
  
  convert.phase <- function(hours) (- hours/24*2*pi)+pi/2  #convert hours to radians in a polar coordinate system
...
if ((max(r) <= 1) & (max(r) > 0.05)) #special kluge for Stacey
  r.range <- c(0,1) else
...
rae <- c(rnorm(10,.3,.2),rnorm(10,.5,.2),rnorm(10,.2,.1))
